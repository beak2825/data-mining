...
Roblox.Client._launchMode = "unknown";
...
Roblox.InstallationInstructions = (function() {
	...
	function loadImages(mode) {
		...
		if (navigator.userAgent.match(/Mac OS X 10[_|\.]5/)) {
			var oldMacDelaySrc = installInstructionsImage.data("oldmacdelaysrc");
			if (installInstructionsImage && typeof oldMacDelaySrc != "undefined") {
				if (installInstructionsImage.hasClass("src-replaced")) {
					installInstructionsImage.attr('src', oldMacDelaySrc);
				} else {
					installInstructionsImage.attr('data-delaysrc', oldMacDelaySrc);
				}
			}
		} else {
			var activationSrc = installInstructionsImage.data("activationsrc");
			if (mode === "activation" && activationSrc !== undefined) {
				if (installInstructionsImage.hasClass("src-replaced")) {
					installInstructionsImage.attr('src', activationSrc);
				} else {
					installInstructionsImage.attr('data-delaysrc', activationSrc);
				}
			}
		}
	}
