...
function be({
	...
	hideSeparator: g = !1,
	showCloseButton: p = !1,
	shouldIgnoreClickOutside: l = !1,
	closeButton: v,
	isScrollable: b = !0,
	position: S = "center",
	removePopoverStyling: L = !1,
	showOverlayBackground: q = !0,
	shadow: T = "normal"
}) {
	const F = !!f,
		R = !!t,
...
function Ee() {
	...
	function s(c) {
		return c = t(c), c && a.test(c) ? c.replace(e, "\\$&") : c
	}
	return O = s, O
}
var Fe = Ee();
...
function U({
	text: t,
	textToHighlight: e,
	shouldAdjustStart: a = !1,
	highlightClassName: s
}) {
	const c = i.useMemo(() => {
			if (!a || !e) return t;
...
function Ae({
	...
	const {
		...
		onSelect: c,
		query: o
	} = t, r = _(), u = Pe(s.updateTime), d = De(s), f = N(x => x.activeIndex) === e, m = i.useRef(null);
	return i.useEffect(() => {
		let x;
		const g = setTimeout(() => {
			x = new IntersectionObserver(p => {
				p.forEach(l => {
					l.isIntersecting && (C.logImpression(s.source, e, s.conversationId), x && x.disconnect())
				})
			}, {
				threshold: .5
		...
		children: n.jsx(E, {
			...
			onClick: () => {
				C.logClick(s.conversationId, e, s.source), c && c()
			},
			onMouseEnter: () => k.setActiveIndex(e),
			onMouseLeave: () => k.setActiveIndex(void 0),
			children: n.jsxs("div", {
...
function $() {
	const t = se(),
		e = V(),
		{
			closeFannyPackModal: a
		} = A();
	return i.useCallback(s => {
		e && pe.closeActiveSidebar(), t(s), a("click")
	}, [t, a, e])
}
...
function Le() {
	const {
		conversations: t
	} = ve(i.useMemo(() => ({}), [])), e = D(), a = G(e, "550560761").value, s = Number(a?.history_results_limit ?? 6), c = i.useMemo(() => t.slice(0, Math.min(t.length, s)), [t, s]), o = ke(c, 0), r = $(), u = i.useMemo(() => ({
		kind: "newChat",
		onSelect: () => {
			r("/")
		}
	...
	return i.useMemo(() => {
		const y = [u];
		return y.push(...[o.recent, o.dynamicMonths, o.dynamicYears].reduce((f, m) => (Object.entries(m).forEach(([x, {
			items: g,
			label: p
		}]) => {
			g.length > 0 && (f.push({
...
function Te() {
	...
	const p = i.useMemo(() => d?.pages.flatMap(h => h.items) ?? [], [d]),
		l = i.useRef(null),
		v = i.useCallback(h => {
			f || h == null || (l.current?.disconnect(), l.current = new IntersectionObserver(w => {
	...
	const b = p && a === t,
		...
		T = Number(q?.local_results_limit ?? 0),
		F = he(t, T),
		R = i.useMemo(() => {
			const h = F;
...
function Be(t, e) {
	...
	i.useEffect(() => {
		const o = r => {
			r.isComposing || (r.key === "ArrowUp" && !r.shiftKey ? k.setActiveIndex(s()) : r.key === "ArrowDown" && !r.shiftKey ? k.setActiveIndex(a()) : (r.key === "Enter" || r.key === "Return") && (r.preventDefault(), c()))
		};
...
function Ke() {
	...
	const y = i.useMemo(() => ({
		...
		f = i.useMemo(() => {
			const l = [];
			return o && d && l.push(y), r ? l.push(...e) : l.push(...c), a ? l.push({
				kind: "loading",
	...
	const x = Qe(m, f),
		g = i.useRef(null);
	i.useEffect(() => {
		k.setActiveIndex(0), g.current && (g.current.scrollTop = 0)
	}, [o]);
	const p = i.useRef(!0);
	return i.useEffect(() => {
		p.current && !a && f.length === 1 && f[0].kind === "noResults" && C.logNoResults(), p.current = a
	}, [a, f]), n.jsx("div", {
		ref: g,
		children: n.jsx("ol", {
			className: "mx-2",
			children: f.map((l, v) => n.jsx("li", {
				ref: x.current[v],
				children: Oe(l, v, v === f.length - 1 ? s : void 0)
			}, v))
		})
	})
}
...
function Ue(t) {
	...
	const e = P.c(9),
		...
		c = _();
	let o;
	e[0] !== s ? (o = j("hover:bg-token-main-surface-secondary focus-visible:ring-token-text-quaternary dark:hover:bg-token-main-surface-tertiary flex items-center justify-center rounded-full bg-transparent p-1 focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:ring-offset-transparent focus-visible:outline-hidden", s), e[0] = s, e[1] = o) : o = e[1];
	let r;
	...
	return e[5] !== a || e[6] !== o || e[7] !== r ? (d = n.jsx("button", {
		...
		onClick: a,
		children: u
	}), e[5] = a, e[6] = o, e[7] = r, e[8] = d) : d = e[8], d
}
...
function Z(t) {
	...
	const e = P.c(19),
		...
		r = D();
	let u;
	e[0] !== r ? (u = Q(r, "3678527908"), e[0] = r, e[1] = u) : u = e[1];
	const d = u;
	let y;
	e[2] !== r ? (y = Q(r, "1422501431"), e[2] = r, e[3] = y) : y = e[3];
	const f = y;
	let m, x;
	e[4] !== c || e[5] !== d || e[6] !== f ? (m = () => {
		!o.current && d && c && (f ? K.setCurrentAccountV2(c) : K.setCurrentAccount(c), o.current = !0)
	}, x = [d, c, f], e[4] = c, e[5] = d, e[6] = f, e[7] = m, e[8] = x) : (m = e[7], x = e[8]), i.useEffect(m, x);
	let g;
	e[9] !== a ? (g = j("flex flex-col", a), e[9] = a, e[10] = g) : g = e[10];
	let p;
	e[11] === Symbol.for("react.memo_cache_sentinel") ? (p = n.jsx(Ne, {}), e[11] = p) : p = e[11];
	let l;
	e[12] !== s ? (l = n.jsxs("div", {
...
function Ve(t) {
	...
	let o;
	e[2] === Symbol.for("react.memo_cache_sentinel") ? (o = n.jsx(Z, {}), e[2] = o) : o = e[2];
	let r;
...
export {
	et as FannyPackModalContainer
};
//# sourceMappingURL=ksc7cmkbpcodyd9o.js.map