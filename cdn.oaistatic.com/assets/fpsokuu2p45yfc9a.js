...
function T(e, s) {
	const [n, u] = p.useState(!1), t = p.useRef({
	...
	}).current, r = V(), d = v(), g = U(), o = L(a => a.isPlaying && a.sourceUrl === c.get(s)?.src), i = p.useCallback(async () => {
		if (!g || !e) return;
		u(!0);
		const a = {
		...
		try {
			const f = await _({
				key: s,
				onStreamingError: b => {
					S.readAloud.error(b, a), u(!1), d.danger(r.formatMessage(A.playbackError, {
						error: b.message
					}), {
			...
			g.changeSource(f), t.playPromise = g.play()
		} catch (f) {
			S.readAloud.error(f, a), u(!1), d.danger(r.formatMessage(A.playbackError, {
				error: f.message
			}), {
				toastId: "useVoiceReadAloudAudio_onStreamingError",
				loggingTitle: A.playbackError.defaultMessage,
		...
		y = {
			sourceopen: async () => {
				...
				if (r.segments.length && !r.streaming) {
					i("streaming from cache"), r.streaming = !0, n();
					for (const a of r.segments) l.appendBuffer(a), await m();
					o.readyState === "open" && o.endOfStream(), r.streaming = !1, i("done streaming from cache");
					return
				}
				if (d) {
					i("fetching audio");
					try {
						...
						for (; c.get(e)?.id === r.id;) {
							...
							if (f) {
								i("done streaming"), r.streaming = !1, o.readyState === "open" && o.endOfStream();
								break
							}
							if (!Array.from(o.sourceBuffers).includes(l)) {
								i("stop streaming, source buffer removed"), r.streaming = !1;
								break
							}
							r.streaming || (i("start streaming"), r.streaming = !0, n()), l.appendBuffer(b), r.segments.push(b), await m()
						}
					} catch (a) {
						if (i("error while streaming", a), r.streaming = !1, o.readyState === "open") try {
			...
			sourceclose: () => {
				i("sourceclose"), r.streaming && (r.streaming = !1, c.get(e)?.id === r.id && (i("sourceclosed while streaming, cleaning up cache"), c.delete(e)))
			},
			sourceended: M
		};
	for (const [l, m] of Object.entries(y)) o.addEventListener(l, () => {
		try {
			return m()
		} catch (a) {
			S.readAloud.error(a)
...
export {
	T as a, h as g, N as u
};
//# sourceMappingURL=fpsokuu2p45yfc9a.js.map