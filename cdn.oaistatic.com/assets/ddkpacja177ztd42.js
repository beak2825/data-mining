...
function tt({
	...
	const {
		icon: u
	} = r, n = "scrub" in r ? r.scrub : [], x = 6, p = o - x * 2, k = I(), E = l.useRef(null), d = l.useRef(null), [w, V] = l.useState(n ? Math.floor(n.length / 2) : 0), C = Math.round(w), N = l.useRef(w), v = n ? k.formatMessage(n[C].title) : null, T = Me(0);
	Ie(T, "change", M => {
		const K = -137 + p / 2,
			D = (be - x) / 2 - p / 2,
			H = et(M, K, D) * (n ? n.length - 1 : 0);
		N.current = H, V(H)
	});
	const A = () => {
			if (!n || qe(n[C]) || f) return c?.();
			n && s?.()
		},
		B = M => {
			M.preventDefault(), !(!n || !f || !("prompt" in n[C])) && m?.(M, k.formatMessage(n[C].prompt))
		};
	return e.jsx(g.div, {
		ref: E,
...
function gt({
	...
	const [k, E] = l.useState(!1), [d, w] = l.useState(!c && !n), [V, C] = l.useState(!0), [N, v] = l.useState(null), [T, A] = l.useState(!1), [B, M] = l.useState(!1), [K, D] = l.useState(!1), [y, H] = l.useState(null), [P, Y] = l.useState(null), [U, q] = l.useState(null), se = !V, ie = $e(), z = s && !d, i = z ? st : ot, re = I(), ae = oe(), Se = Xe(), j = l.useRef(null), S = n || k, F = n && !d && V;
	l.useEffect(() => {
		d && !B && M(!0), f?.(d)
	}, [d, B]), l.useEffect(() => {
		T && !S && !y && (j.current != null && clearTimeout(j.current), w(!0)), !T && !S && !y && (j.current != null && clearTimeout(j.current), j.current = window.setTimeout(() => {
			w(!1), v(null), D(!1)
		}, ye))
	}, [T, S, y, d]), l.useEffect(() => {
		if (n) {
		...
		E(!1), !B && !c && (M(!0), w(!0), j.current != null && clearTimeout(j.current), j.current = window.setTimeout(() => {
			w(!1), v(null), D(!1), M(!0)
		}, ye))
	}, [n, c, B]);
	const ee = t => {
		...
		X = (t, a, b) => {
			A(!1), E(!0), w(!1), v(null), D(!1), H(null), Y(null), q(null);
			const Q = Se(),
				xe = (O, Z) => {
		...
		ce = (t, a) => {
			...
			if ("scrub" in a) return H(b);
			if ("menu" in a) return Y(b);
			if ("grid" in a) return q(b);
			ee(b)
		},
		le = (t, a) => {
			se || "prompt" in a && X(t, re.formatMessage(a.prompt), a)
		},
		Ee = ke.map(t => {
			if (!d) return null;
			const {
	...
	l.useEffect(() => {
		...
		if (t) return Ze(document, {
			mousedown: ({
				target: a
			}) => {
				a instanceof HTMLElement && !t.contains(a) && (w(!1), v(null), D(!1), A(!1), H(null), Y(null), q(null))
			}
		})
	}, [y, P, U]);
	const de = Math.round(W / i) * 2,
		me = [];
	let fe = 0;
	for (let t = 0; t < de - 1; t++) fe += W / de, me.push(e.jsx(rt, {
		top: fe
	}));
	const he = e.jsx(g.div, {
		...
		animate: {
			scale: S || F || k ? .85 : 1,
			opacity: x ? 0 : 1,
			borderRadius: P && R && !S || U && _ && !S ? it : i / 2
		},
		onHoverStart: () => {
		...
		onHoverEnd: () => A(!1),
		children: e.jsx(pe, {
			children: P && R && "menu" in R && !S ? e.jsx(g.div, {
				initial: {
					height: i,
					width: i
				},
				...
				children: e.jsx(Ye, {
					isConfirming: N === R.id,
					onConfirm: () => ee(R.id),
					onSubmit: (t, a) => X(t, a, R),
					menu: R.menu
				})
			}) : U && _ && "grid" in _ && !S ? e.jsx(g.div, {
				initial: {
					height: i,
					width: i
				},
				...
				children: [e.jsx(pe, {
					...
					children: S ? F ? e.jsx(je, {
						title: at.cancel,
						icon: e.jsx(De, {
							className: G("animate-pulsing m-auto shrink-0", z ? "h-6 w-6" : "h-10 w-10")
						}),
						isSmall: z,
						opacity: 1,
						size: i,
...
export {
	gt as D, pt as a, Ke as g, ze as i, Xe as u
};
//# sourceMappingURL=ddkpacja177ztd42.js.map