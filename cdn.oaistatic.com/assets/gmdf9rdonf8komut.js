...
class L extends O {
	...
	lineInner(t, e, i, s) {
		for (let r = 0;; r++) {
			let o = this.text[r],
				l = s + o.length;
			if ((e ? i : l) >= t) return new vr(s, l, i, o);
			s = l + 1, i++
		}
	}
	decompose(t, e, i, s) {
...
function Js(n, t, e) {
	...
	for (t += si(i); t < n.length;) {
		let s = te(n, t);
		if (i == Qi || s == Qi || e && Sr(s)) t += si(s), i = s;
		else if (Ji(s)) {
			let r = 0,
				o = t - 2;
	...
	static of (t, e, i) {
		...
		function f(a) {
			if (Array.isArray(a))
				...
				if (c > u || c < 0 || u > e) throw new RangeError(`Invalid change range ${c} to ${u} (in doc of length ${e})`);
...
class v {
	...
	static fromJSON(t) {
		...
		return new v(t.ranges.map(e => Ct.fromJSON(e)), t.main)
	}
	static single(t, e = t) {
	...
	static create(t, e = 0) {
		...
		for (let i = 0, s = 0; s < t.length; s++) {
			let r = t[s];
			if (r.empty ? r.from <= i : r.from < i) return v.normalized(t.slice(), e);
			i = r.to
		}
		return new v(t, e)
	}
	...
	static normalized(t, e = 0) {
		...
		for (let s = 1; s < t.length; s++) {
			...
			if (r.empty ? r.from <= o.to : r.from < o.to) {
				...
				s <= e && e--, t.splice(--s, 2, r.anchor > r.head ? v.range(h, l) : v.range(l, h))
			}
		}
		return new v(t, e)
	}
...
class B {
	...
	static create(t = {}) {
		let e = Ie.resolve(t.extensions || [], new Map),
			...
			s = t.selection ? t.selection instanceof v ? t.selection : v.single(t.selection.anchor, t.selection.head) : v.single(0);
		...
		if (a.nodeType == 1) {
			...
			if (s == "nearest") t.top < u.top ? (w = -(u.top - t.top + o), e > 0 && t.bottom > u.bottom + w && (w = t.bottom - u.bottom + w + o)) : t.bottom > u.bottom && (w = t.bottom - u.bottom + o, e < 0 && t.top - w < u.top && (w = -(u.top + w - t.top + o)));
				...
				let y = t.bottom - t.top,
					S = u.bottom - u.top;
				w = (s == "center" && y <= S ? t.top + y / 2 - S / 2 : s == "start" || s == "center" && e < 0 ? t.top - o : t.bottom - S + o) - u.top
			}
			if (i == "nearest" ? t.left < u.left ? (p = -(u.left - t.left + r), e > 0 && t.right > u.right + p && (p = t.right - u.right + p + r)) : t.right > u.right && (p = t.right - u.right + r, e < 0 && t.left < u.left + p && (p = -(u.left + p - t.left + r))) : p = (i == "center" ? t.left + (t.right - t.left) / 2 - (u.right - u.left) / 2 : i == "start" == l ? t.left - r : t.right - (u.right - u.left) + r) - u.left, p || w)
				if (d) f.scrollBy(p, w);
					...
					if (w) {
						let x = a.scrollTop;
						a.scrollTop += w / m, S = (a.scrollTop - x) * m
					}
					if (p) {
						let x = a.scrollLeft;
					...
					t = {
						...
						right: t.right - y,
						bottom: t.bottom - S
					}, y && Math.abs(y - p) < 1 && (i = "nearest"), S && Math.abs(S - w) < 1 && (s = "nearest")
				} if (d) break;
			a = a.assignedSlot || a.parentNode
...
function yn(n, t, e, i, s, r, o, l, h) {
	...
	if (!(t == i && a && !o && !u && r.length < 2 && a.merge(e, s, r.length ? c : null, e == 0, l, h))) {
		if (i < f.length) {
			let d = f[i];
			d && (s < d.length || d.breakAfter && c?.breakAfter) ? (t == i && (d = d.split(s), s = 0), !u && c && d.merge(0, s, c, !0, 0, h) ? r[r.length - 1] = d : ((s || d.children.length && !d.children[0].length) && d.merge(0, s, null, !1, 0, h), r.push(d))) : d?.breakAfter && (c ? c.breakAfter = 1 : o = 1), i++
		}
...
const bi = /Edge\/(\d+)/.exec(Y.userAgent),
	...
	Sn = !$e && /Apple Computer/.test(Y.vendor),
	fs = Sn && (/Mobile\/\w+/.test(Y.userAgent) || Y.maxTouchPoints > 2);
var b = {
	mac: fs || /Mac/.test(Y.platform),
	windows: /Win/.test(Y.platform),
	linux: /Linux|X11/.test(Y.platform),
	ie: $e,
	ie_version: vn ? wi.documentMode || 6 : yi ? +yi[1] : bi ? +bi[1] : 0,
	gecko: as,
...
class N extends R {
	...
	sync(t, e) {
		var i;
		this.dom ? this.flags & 4 && (gn(this.dom), this.dom.className = "cm-line", this.prevAttrs = this.attrs ? null : void 0) : (this.setDOM(document.createElement("div")), this.dom.className = "cm-line", this.prevAttrs = this.attrs ? null : void 0), this.prevAttrs !== void 0 && (vi(this.dom, this.prevAttrs, this.attrs), this.dom.classList.add("cm-line"), this.prevAttrs = void 0), super.sync(t, e);
...
function ro(n, t, e, i, s) {
	...
	return c && a == f && c.level + (s ? 0 : 1) < h.level ? v.cursor(c.side(!s, e) + n.from, c.forward(s, e) ? 1 : -1, c.level) : v.cursor(a + n.from, h.forward(s, e) ? -1 : 1, h.level)
}
...
class Nt {
	...
	clip(t) {
		return this.range.to <= t.doc.length ? this : new Nt(v.cursor(t.doc.length), this.y, this.x, this.yMargin, this.xMargin, this.isSnapshot)
	}
}
const ve = F.define({
		map: (n, t) => n.map(t)
...
function mo(n, t, e = 1) {
	let i = n.charCategorizer(t),
		s = n.doc.lineAt(t),
		r = t - s.from;
	if (s.length == 0) return v.cursor(t);
...
function Oi(n, t, e) {
	...
	for (let g = n.firstChild; g; g = g.nextSibling) {
		...
		for (let p = 0; p < m.length; p++) {
			...
			let y = wo(t, w),
				S = bo(e, w);
			if (y == 0 && S == 0) return g.nodeType == 3 ? ws(g, t, e) : Oi(g, t, e);
			if (!i || o > S || o == S && r > y) {
				i = g, s = w, r = y, o = S;
				let x = S ? e < w.top ? -1 : 1 : y ? t < w.left ? -1 : 1 : 0;
...
function Xn(n, t, e, i = -1) {
	...
	} else return S.localPosFromDOM(w, y) + S.posAtStart
}
...
function vo(n, t, e, i) {
	...
	if (r) {
		...
		if (h != null) return v.cursor(h, e ? -1 : 1)
	}
...
function ko(n, t, e, i) {
	let s = t.head,
		r = e ? 1 : -1;
	if (s == (e ? n.state.doc.length : 0)) return v.cursor(s, t.assoc);
	let o = t.goalColumn,
...
class Do {
	...
	move(t) {
		...
		let h = Gn(this.view);
		t.clientX - h.left <= s + Se ? e = -ke(s - t.clientX) : t.clientX + h.right >= o - Se && (e = ke(t.clientX - o)), t.clientY - h.top <= r + Se ? i = -ke(r - t.clientY) : t.clientY + h.bottom >= l - Se && (i = ke(t.clientY - l)), this.setScrollSpeed(e, i)
	}
	up(t) {
	...
	if (Ti != null && e.selection.ranges.every(h => h.empty) && Ti == r.toString()) {
		...
		i = e.changeByRange(f => {
			...
			return {
				...
				range: v.cursor(f.from + c.length)
			}
		})
...
let _n = (n, t) => n >= t.top && n <= t.bottom,
	Ss = (n, t, e) => _n(t, e) && n >= e.left && n <= e.right;
...
function Lo(n, t, e, i) {
	...
	if (o && Ss(e, i, o)) return -1;
	let l = s.coordsAt(r, 1);
	return l && Ss(e, i, l) ? 1 : o && _n(i, o) ? -1 : 1
}
...
function Vo(n, t) {
	...
	return {
		...
		get(r, o, l) {
			let h = ks(n, r),
				f, a = vs(n, h.pos, h.bias, i);
			if (e.pos != h.pos && !o) {
				let c = vs(n, e.pos, e.bias, i),
					u = Math.min(c.from, a.from),
					d = Math.max(c.to, a.to);
				a = u < a.from ? v.range(u, d) : v.range(d, u)
			}
			return o ? s.replaceRange(s.main.extend(a.from, a.to)) : l && i == 1 && s.ranges.length > 1 && (f = Io(s, h.pos)) ? f : l ? s.addRange(a) : v.create([a])
		}
	}
}
...
function Io(n, t) {
	for (let e = 0; e < n.ranges.length; e++) {
		...
		if (i <= t && s >= t) return v.create(n.ranges.slice(0, e).concat(n.ranges.slice(e + 1)), n.mainIndex == e ? 0 : n.mainIndex - (n.mainIndex > e ? 1 : 0))
	}
	return null
}
_.dragstart = (n, t) => {
...
class Go {
	...
	comparePoint(t, e, i, s) {
		(t < e || i && i.heightRelevant || s && s.heightRelevant) && Si(t, e, this.changes, 5)
	}
}
...
class Es {
	...
	measure(t) {
		...
		return S && (f & 2 && (f |= this.updateScaler()), this.viewport = this.getViewport(a, this.scrollTarget), f |= this.updateForViewport()), (f & 2 || S) && this.updateViewportLines(), (this.lineGaps.length || this.viewport.to - this.viewport.from > 4e3) && this.updateLineGaps(this.ensureLineGaps(o ? [] : this.lineGaps, t)), f |= this.computeVisibleRanges(), this.mustEnforceCursorAssoc && (this.mustEnforceCursorAssoc = !1, t.docView.enforceCursorAssoc()), f
	}
	get visibleTop() {
		...
		let l = [],
			h = (a, c, u, d) => {
				...
				if (!p) {
					if (c < u.to && e && i && e.visibleRanges.some(w => w.from <= c && w.to >= c)) {
						let w = e.moveToLineBoundary(v.cursor(c), !1, !0).head;
						w > a && (c = w)
					}
			...
			f = a => {
				...
				if (i) {
					...
					if (u != null) {
						let y = Ae(c, u),
							S = ((this.visibleBottom - this.visibleTop) / 2 + m) / a.height;
						p = y - S, w = y + S
...
function hr(n, t) {
	...
	if (!e && t.typeOver && !s.empty && i && i.main.empty ? e = {
		...
		} : (b.mac || b.android) && e && e.from == e.to && e.from == s.head - 1 && /^\. ?$/.test(e.insert.toString()) && n.contentDOM.getAttribute("autocorrect") == "off" ? (i && e.insert.length == 2 && (i = v.single(i.main.anchor - 1, i.main.head - 1)), e = {
			from: s.from,
			to: s.to,
			insert: O.of([" "])
		}) : b.chrome && e && e.from == e.to && e.from == s.head && e.insert.toString() == `
 ` && n.lineWrapping && (i && (i = v.single(i.main.anchor - 1, i.main.head - 1)), e = {
			from: s.from,
			to: s.to,
			insert: O.of([" "])
		}), e) return ar(n, e, i, r);
	...
	if (t.from >= r.from && t.to <= r.to && t.to - t.from >= (r.to - r.from) / 3 && (!e || e.main.empty && e.main.from == t.from + t.insert.length) && n.inputState.composing < 0) {
		...
		if (s.selection.ranges.length > 1 && n.inputState.composing >= 0 && t.to <= r.to && t.to >= r.to - 10) {
			...
			i = s.changeByRange(g => {
				...
				return {
					changes: w,
					range: h ? v.range(Math.max(0, h.anchor + y), Math.max(0, h.head + y)) : g.map(w)
				}
			})
		} else i = {
			changes: l,
...
`))
			...
			r.from == this.from && s < this.from ? r.from = s : r.to == this.to && s > this.to && (r.to = s), !(r.from == r.to && !r.insert.length) && (this.pendingContextChange = r, ar(t, r, v.single(this.toEditorPos(i.selectionStart), this.toEditorPos(i.selectionEnd))), this.pendingContextChange && (this.revertPending(t.state), this.setSelection(t.state)))
...
class D {
		...
		try {
			...
			for (let u of t) {
				if (c && (c = c.map(u.changes)), u.scrollIntoView) {
					...
					c = new Nt(d.empty ? d : v.cursor(d.head, d.head > d.anchor ? -1 : 1))
				}
				for (let d of u.effects) d.is(ve) && (c = d.value.clip(this.state))
			}
	...
	updateAttrs() {
		...
		let i = this.observer.ignore(() => {
			let s = vi(this.contentDOM, this.contentAttrs, e),
				r = vi(this.dom, this.editorAttrs, t);
			return s || r
		});
	...
	moveByGroup(t, e) {
		return Je(this, t, ys(this, t, e, i => So(this, t.head, i)))
	}
	visualLineSide(t, e) {
		let i = this.bidiSpans(t),
			s = this.textDirectionAt(t.from),
			r = i[e ? i.length - 1 : 0];
		return v.cursor(r.side(e, s) + t.from, r.forward(!e, s) ? 1 : -1)
	}
	...
	static scrollIntoView(t, e = {}) {
		return ve.of(new Nt(typeof t == "number" ? v.cursor(t) : t, e.y, e.x, e.yMargin, e.xMargin))
	}
	scrollSnapshot() {
		let {
			scrollTop: t,
		...
		return ve.of(new Nt(v.cursor(i.from), "start", "start", i.top - t, e, !0))
	}
	setTabFocusMode(t) {
...
function yl(n, t, e) {
	...
	if (m && (n.lineWrapping || d.widgetLineBreaks) && (m = Fs(n, i, 1, m)), p && (n.lineWrapping || g.widgetLineBreaks) && (p = Fs(n, s, -1, p)), m && p && m.from == p.from && m.to == p.to) return y(S(e.from, e.to, m));
	{
		let k = m ? S(e.from, null, m) : x(d, !1),
			M = p ? S(null, e.to, p) : x(g, !0),
			P = [];
	...
	function S(k, M, P) {
		...
		function Xi(vt, gt, Tt, St, $t) {
			let rt = n.coordsAtPos(vt, vt == P.to ? -2 : 2),
				ot = n.coordsAtPos(Tt, Tt == P.from ? 2 : -2);
			!rt || !ot || (W = Math.min(rt.top, ot.top, W), dt = Math.max(rt.bottom, ot.bottom, dt), $t == I.LTR ? pe.push(r && gt ? c : rt.left, r && St ? u : ot.right) : pe.push(!r && St ? c : ot.left, !r && gt ? u : rt.right))
		}
		let me = k ?? P.from,
			we = M ?? P.to;
		for (let vt of n.visibleRanges)
			if (vt.to > me && vt.from < we)
				for (let gt = Math.max(vt.from, me), Tt = Math.min(vt.to, we);;) {
					let St = n.state.doc.lineAt(gt);
					for (let $t of n.bidiSpans(St)) {
						let rt = $t.from + St.from,
							ot = $t.to + St.from;
						if (rt >= Tt) break;
						ot > gt && Xi(Math.max(rt, gt), k == null && rt <= me, Math.min(ot, Tt), M == null && ot >= we, $t.dir)
					}
					if (gt = St.to + 1, gt >= Tt) break
				}
...
function cr(n) {
	return [nt.define(t => new vl(t, n)), Le.of(n)]
}
const ur = !b.ios,
	ce = C.define({
		combine(n) {
...
function Zl(n = {}) {
	return [ce.of(n), Sl, kl, Cl, Fn.of(!0)]
}
		...
		writeMeasure(n) {
			...
			for (let l = 0; l < this.manager.tooltips.length; l++) {
				...
				let d = h.arrow ? f.dom.querySelector(".cm-tooltip-arrow") : null,
					...
					S = u.width > i.right - i.left ? y ? i.left : i.right - u.width : y ? Math.min(c.left - (d ? 14 : 0) + w.x, i.right - m) : Math.max(i.left, c.left - m + (d ? 14 : 0) - w.x),
					x = this.above[l];
				...
				let M = x ? c.top - p - g - w.y : c.bottom + g + w.y,
					P = S + m;
				if (f.overlap !== !0)
					for (let W of o) W.left < P && W.right > S && W.top < M + p && W.bottom > M && (M = x ? W.top - p - 2 - g : W.bottom + g + 2);
				if (this.position == "absolute" ? (a.style.top = (M - n.parent.top) / r + "px", a.style.left = (S - n.parent.left) / s + "px") : (a.style.top = M / r + "px", a.style.left = S / s + "px"), d) {
					let W = c.left + (y ? w.x : -w.x) - (S + 14 - 7);
					d.style.left = W / s + "px"
				}
				f.overlap !== !0 && o.push({
					left: S,
...
export {
	Kt as A, Zl as B, lt as C, V as D, D as E, C as F, xt as G, U as M, Fi as P, le as R, F as S, j as T, nt as V, jt as W, v as a, qt as b, Wi as c, Vr as d, B as e, Ht as f, ih as g, A as h, I as i, te as j, eh as k, ht as l, si as m, gl as n, Jl as o, It as p, O as q, st as r, Wl as s, H as t, at as u, sh as v, th as w, _l as x, nh as y, Ke as z
};
//# sourceMappingURL=gmdf9rdonf8komut.js.map