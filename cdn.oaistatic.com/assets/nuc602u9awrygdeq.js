...
function ie({
	...
	const t = _(),
		i = S(V.hasSeenAdvancedVoiceNuxFullPage),
		f = z(),
		n = f?.isPlus() ?? !1,
		c = f?.isEnterprisey() ?? !1,
		r = !n && !c,
	...
	const A = () => {
			v(!0), U.voiceAdvancedDisclosureAccepted.click(), x(!0), setTimeout(async () => {
				P(!1)
			}, ee)
		},
		j = () => {
...
function ce() {
	const s = S(V.hasSeenAdvancedVoiceNuxFullPage);
...
function pe({
	...
	return o.useEffect(() => {
		const a = sessionStorage.getItem("selectedVoiceName"),
			g = async v => {
				try {
					sessionStorage.removeItem("selectedVoiceName"), await r({
						voice: v,
					...
					K.addError(`Failed to start voice mode after voice picker: ${p}`, {
						protocol: h
					})
				}
			};
		a && !c && (u(a), g(a))
	}, [n, c, u, r, l, h]), o.useEffect(() => {
		x || i.setState(a => {
			a.showNuxFullPage = !1
		})
	}, [x, i]), t ? e.jsx(ie, {
...
//# sourceMappingURL=nuc602u9awrygdeq.js.map